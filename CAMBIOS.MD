...
## Cambios realizados (P0 - slice 2: baseline + SLA de performance)
- Se agrego script reproducible de baseline:
- `scripts/perfBaseline.mjs`
- Se agrego comando:
- `npm run perf:baseline`
- Se generan artefactos versionados por build:
- `docs/performance/baseline.json`
- `docs/performance/baseline.md`
- Se documento SLA tecnico inicial y protocolo de medicion en:
- `docs/performance-sla.md`

## Cambios realizados (P0 - slice 3: contrato de eventos progress:*)
- Se creo bus central de eventos con schema validation (Zod):
- `src/lib/events/progressEventBus.js`
- Se definio catalogo unico para eventos principales:
- `progress:dataUpdated`
- `progress:challengeCompleted`
- `progress:navigate`
- `progress:srs-updated`
- Se migro emision de eventos criticos a helper central:
- `src/lib/progress/tracking.js`
- `src/lib/progress/challenges.js`
- Se migro consumo de eventos criticos a helper central:
- `src/features/progress/useProgressDashboardData.js`
- `src/features/progress/ProgressDashboard.jsx`
- `src/lib/cache/ProgressDataCache.js`
- Se documento contrato y regla de uso:
- `docs/events/progress-events.md`
- Test agregado:
- `src/lib/events/progressEventBus.test.js`

## Cambios realizados (P0 - slice 4: E2E de navegacion critica)
- Se agrego suite Playwright dedicada para flujos criticos de navegacion:
- `tests/e2e/navigation-critical.e2e.js`
- Escenarios cubiertos:
- onboarding end-to-end (ruta de ingreso hasta Drill).
- back/forward del navegador durante onboarding.
- entrada a Drill, navegacion a Progress y retorno con historial.
- Se agrego helper de reset de estado local (localStorage/sessionStorage/IndexedDB) para reducir flakes por estado residual entre corridas.
- Validacion ejecutada:
- `npx playwright test tests/e2e/navigation-critical.e2e.js --project=chromium` en verde.

## Cambios realizados (P0 - slice 5: migracion de progress:navigate al event bus)
- Se reemplazo emision ad-hoc de `progress:navigate` por helper central `emitProgressEvent(...)` en componentes clave.
- Archivos migrados:
- `src/features/progress/SRSPanel.jsx`
- `src/features/progress/ErrorRadar.jsx`
- `src/features/progress/VerbMasteryMap.jsx`
...
- `src/features/progress/CompetencyRadar.jsx`
- `src/features/progress/HeatMap.jsx`
- `src/features/progress/PracticeRecommendations.jsx`
- `src/features/drill/SessionHUD.jsx`
- Resultado:
- menor dispersión de contratos de evento,
- validacion de payload consistente,
- migracion alineada con catalogo `docs/events/progress-events.md`.

## Cambios realizados (P0 - slice 6: contrato canonico de rutas + ADR)
- Se definio contrato unico de routing en:
- `src/lib/routing/routeContract.js`
- Reglas formalizadas:
- modos validos centralizados (`onboarding`, `drill`, `learning`, `progress`, `story`, `timeline`).
- `step` solo permitido para `onboarding` (se ignora en otros modos).
- Se refactorizo `Router` para consumir el contrato central:
- parseo URL (`parseRouteFromURL`),
- normalizacion (`normalizeRoute`),
- construccion de URL canónica (`buildRouteURL`).
- Se creo ADR de decision arquitectonica:
- `docs/adr/0001-routing-single-source-of-truth.md`
- Test agregado:
- `src/lib/routing/routeContract.test.js`

## Cambios realizados (P0 - slice 7: migracion de rutas criticas + regresion URL)
- Se migraron puntos criticos de navegacion a factories del contrato (`ROUTES`):
- `src/components/AppRouter.jsx`
- `src/hooks/useOnboardingFlow.js`
- `src/lib/routing/Router.js`
- Rutas críticas cubiertas explícitamente con contrato único:
- `home` (`onboarding/2`)
- `onboarding` (pasos)
- `drill`
- `progress`
- Se reforzo `ensureHistoryState` para canonizar URL cuando hay path o query legado (ej: `?mode=progress`).
- Se amplio E2E en navegacion critica:
- `tests/e2e/navigation-critical.e2e.js`
- escenario nuevo de normalizacion de URLs legacy/no canonicas y validacion de estado de ruta.
- Resultado validado:
- `npx playwright test tests/e2e/navigation-critical.e2e.js --project=chromium` en verde con 4 escenarios.

## Cambios realizados (P0 - slice 8: guard runtime para keys desconocidas en settings)
- Se agrego hardening en `useSettings` para rechazar claves desconocidas al hacer `set(...)`:
- `src/state/settings.js`
- Se incorporo sanitizacion de updates:
- solo se aplican claves definidas en el schema de estado base,
- claves desconocidas se ignoran (con warning en DEV).
- Se simplifico `set` público para aplicar solo patch parcial (evita merges de estado completo innecesarios).
- Se exporto helper de test para resetear estado de settings:
- `resetSettingsForTests()`.
- Tests nuevos:
- `src/state/settings.test.js`
- cubren bloqueo de claves desconocidas y preservación de métodos del store.

## Cambios realizados (P0 - slice 9: migracion de estado efimero drill a session store)
- Se extendio `useSessionStore` como fuente runtime para contexto efimero de Drill:
- `runtimeCurrentBlock`
- `runtimeReviewSessionType`
- `runtimeReviewSessionFilter`
- API agregada:
- `setDrillRuntimeContext(...)`
- `clearDrillRuntimeContext()`
- `getRuntimeDrillSettings(baseSettings)`
- Archivo:
- `src/state/session.js`
- El generador ahora compone settings persistidos + contexto efimero de sesion:
- `src/hooks/modules/useDrillGenerator.js`
- Se migraron writers de contexto efimero en UI para evitar depender de `useSettings` en `currentBlock`/`reviewSessionFilter`:
- `src/components/AppRouter.jsx`
...
- `src/features/progress/ErrorIntelligence.jsx`
- `src/features/progress/EnhancedErrorAnalysis.jsx`
- Se documento checklist/auditoria de claves persistentes vs efimeras:
- `docs/state/settings-ephemeral-audit.md`
- Test nuevo:
- `src/state/session.test.js`

## Cambios realizados (P0 - slice 10: versionado y migraciones formales de settings)
- Se establecio pipeline explícito de migraciones por versión en `useSettings`:
- `src/state/settings.js`
- Se incrementó versión del store persistido:
- `SETTINGS_VERSION: 3`
- Nueva API de migración:
- `migratePersistedSettings(persistedState, fromVersion)`
- Migraciones implementadas:
- eliminación de claves legacy de runtime Drill (`currentBlock`, `reviewSessionType`, `reviewSessionFilter`) desde snapshots viejos.
- normalización de alias regional legacy (`both` -> `global`).
- normalización de `practiceReminderDays` con valores legacy mixtos.
- Tests de migración agregados:
- `src/state/settings.migration.test.js`

## Cambios realizados (P0 - slice 11: snapshots historicos y migracion tolerante)
- Se reforzó sanitización de snapshots persistidos para evitar pérdida total de configuración válida si una sola key viene corrupta.
- Cambio clave en `src/state/settings.js`:
- validación por campo (key-by-key) en lugar de invalidar snapshot completo por un error puntual.
- Se agregaron casos de regresión con snapshots históricos representativos:
- `src/state/settings.migration.test.js`
- Casos cubiertos:
- preservación de campos válidos cuando otro campo viene malformado.
- carga de snapshot tipo v1 con normalizaciones esperadas sin perder config útil.
- remoción de claves legacy efímeras de drill en migración.
- Con esto, la apertura de snapshots viejos ahora mantiene configuración válida en lugar de resetear a defaults por un único campo inválido.

## Cambios realizados (P0 - slice 12: cola serializada para persistencia de settings)
- Se reemplazo debounce ad-hoc de persistencia por una cola serializada reutilizable:
- `src/state/settingsPersistence.js`
- Integracion en `useSettings`:
- `src/state/settings.js`
- Mejoras aplicadas:
- coalescing de cambios rapidos (persiste solo el snapshot mas reciente tras debounce).
- `flushSettings()` ahora reutiliza la misma cola y respeta orden de escritura.
- serializacion de guardados en vuelo para evitar escrituras fuera de orden.
- skip de escrituras obsoletas por `lastUpdated` ya persistido.
- Test nuevo:
- `src/state/settingsPersistence.test.js`
- cubre debounce, flush inmediato y serializacion con request en vuelo.

## Cambios realizados (P0 - slice 13: guard anti-stale en save de settings + invariantes)
- Se agrego protección contra escrituras fuera de orden en:
- `src/lib/progress/database.js` (`saveUserSettings`)
- Regla:
- si llega un snapshot con `lastUpdated` más viejo que el ya persistido, se ignora y se preserva el registro más nuevo.
- Esto reduce write-races entre debounce local, flush y merges/sync.
- Test agregado:
- `src/lib/progress/database.userSettingsRace.test.js`
- casos cubiertos:
- snapshot viejo no sobreescribe snapshot nuevo.
- snapshot nuevo sí sobreescribe correctamente.
- Se documentaron invariantes operativas de escritura/orden:
- `docs/state/settings-write-invariants.md`
- incluyendo pipeline único (`schedule` + `flush`), serialización, anti-stale y orden de sync.
...
## Cambios realizados (P11 - slice 3: cooldown de acciones en tarjetas de recordatorio)
- `PracticeReminders` ahora usa `createActionCooldown` para evitar doble-disparo rapido en acciones:
- `Activar recordatorio diario`.
- `Practicar ahora`.
- Se agrega cleanup del cooldown al desmontar el componente.
- Tests agregados/actualizados:
- `src/features/progress/PracticeReminders.test.jsx` (nuevo caso de clicks rapidos coalescidos)
- `src/features/progress/actionCooldown.test.js`

## Estado P11
- P11 finalizado (slices 1 a 3 completos y documentados).

## Cambios realizados (P12 - slice 1: indice precomputado por combinacion en generador)
- Se agrego un indice precomputado de formas por combinacion en `src/hooks/modules/formsPoolService.js`:
- `byMoodTense` para consultas por `mood|tense`.
- `byMoodTensePerson` para consultas por `mood|tense|person`.
- `resolveFormsPool` ahora construye y cachea ese indice junto al pool resuelto, evitando recomputar indexaciones en llamadas sucesivas con la misma firma.
- Se integro el indice como dependencia opcional del pipeline de filtros:
- `applyComprehensiveFiltering(forms, settings, specificConstraints, formsIndex)`
- `filterForSpecificPractice(allForms, specificConstraints, formsIndex)`
- `useDrillGenerator` pasa `poolResult.index` al filtro especifico para usar fast-path sin reescanear todo el arreglo en cada intento.
- Beneficio: menor costo CPU en escenarios con filtros especificos y generacion repetida sobre el mismo pool.
- Tests agregados/actualizados:
- `src/hooks/modules/formsPoolService.test.js`
- `src/hooks/modules/DrillFormFilters.test.js`
- `src/hooks/modules/useDrillGenerator.generateNextItem.test.js`

## Cambios realizados (P12 - slice 2: indice consciente de region sin romper contratos)
- Se extendio `createFormsCombinationIndex` para mantener buckets regionales adicionales:
- `byRegionMoodTense`
- `byRegionMoodTensePerson`
- `resolveFormsPool` ahora inyecta `defaultRegion` al construir/reconstruir el index, de modo compatible con caches previas.
- `buildFormsPool` normaliza las formas con `region` de origen para que el index pueda discriminar por dialecto.
- `filterForSpecificPractice` y `getSpecificPracticeCandidates` aceptan region opcional y priorizan buckets regionales cuando existen.
- `applyComprehensiveFiltering` pasa `settings.region` a la etapa de filtro especifico para habilitar fast-path regional sin cambiar el contrato externo.
- Beneficio: el filtro especifico evita escaneos innecesarios y queda listo para escenarios multi-region sin afectar llamadas actuales.
- Tests agregados/actualizados:
- `src/hooks/modules/formsPoolService.test.js`
- `src/hooks/modules/DrillFormFilters.test.js`
- `src/hooks/modules/useDrillGenerator.generateNextItem.test.js`

## Cambios realizados (P12 - slice 3: benchmark reproducible de impacto del index en generador)
- Se agrego script reproducible de benchmark para el filtro especifico del generador:
- `scripts/perfGeneratorBenchmark.mjs`
- Se agrego comando npm para correrlo de forma estandar:
- `npm run perf:generator`
- El benchmark persiste resultados versionables en:
- `docs/performance/generator-benchmark.json`
- `docs/performance/generator-benchmark.md`
- El benchmark compara escenario sin index vs con index precomputado (misma configuracion, mismas formas, p50/p95).
- Baseline actual registrado (2026-02-12):
- `formsCount: 22412`
- `p95 sin indice: 0.392ms`
- `p95 con indice: 0ms` (redondeado a 3 decimales)
- `delta p95: 0.392ms (100%)`

## Estado P12
- P12 finalizado (slices 1 a 3 completos y documentados).

## Cambios realizados (P13 - slice 1: motivos estandar de descarte en pipeline de filtros)
- Se estandarizaron motivos de descarte en `src/hooks/modules/DrillFormFilters.js` mediante `FILTER_DISCARD_REASONS`.
- Se agrego `getFilteringDiagnostics(forms, settings, specificConstraints, formsIndex)` para obtener:
- `filtered`
- `stages` con `before`, `after`, `dropped`, `skipped`, `reason`
- `emptyReason` canonico cuando el pool queda vacio.
- `applyComprehensiveFiltering` ahora delega en un pipeline unico interno para evitar divergencias entre filtrado y diagnostico.
- `useDrillGenerator` guarda y expone `lastFilteringReport` con el ultimo reporte causal del filtrado.
- En `generateNextItem`, cuando no hay elegibles, el warning de fallback incluye el reporte causal (si existe) para depuracion.
- Tests agregados/actualizados:
- `src/hooks/modules/DrillFormFilters.test.js`
- `src/hooks/modules/useDrillGenerator.generateNextItem.test.js`

## Cambios realizados (P13 - slice 2: diagnostico causal visible en Drill con acciones guiadas)
- Se conecto el diagnostico causal del generador con la UI de Drill:
- `src/components/drill/DrillMode.jsx` ahora consume `stats.lastFilteringReport`.
- `buildGenerationDetail` y `buildGenerationSuggestions` pasan a ser reason-aware cuando existe `emptyReason`.
- Se agrego mapeo de causas canonicas a mensajes y acciones recomendadas en:
- `src/components/drill/generationDiagnostics.js`
- Cuando el pool queda vacio, Drill muestra `Causa detectada` y prioriza sugerencias relevantes para desbloquear la sesion.
- Tests agregados/actualizados:
- `src/components/drill/generationDiagnostics.test.js`

## Cambios realizados (P13 - slice 3: cobertura del flujo causal en unit + integracion de generador)
- Se amplio cobertura para validar el flujo causal end-to-end del pipeline de filtrado:
- `src/hooks/modules/DrillFormFilters.test.js` valida `emptyReason` estandar y detalle de stage.
- `src/hooks/modules/useDrillGenerator.generateNextItem.test.js` valida exposicion de `lastFilteringReport` desde `getGenerationStats`.
- Se ajusto `useDrillGenerator` para retornar en stats el reporte causal mas reciente del ciclo actual sin depender del timing async de React state.

## Estado P13
- P13 finalizado (slices 1 a 3 completos y documentados).

## Cambios realizados (P14 - slice 1: baseline formal para benchmark de generador)
- Se agregaron utilidades para formalizar baseline de performance del generador:
- `scripts/perfGeneratorSetBaseline.mjs`
- `scripts/perfGeneratorCheck.mjs`
- Nuevos comandos npm:
- `npm run perf:generator:baseline`
- `npm run perf:generator:check`
- Se versiono baseline actual en:
- `docs/performance/generator-baseline.json`
- Se actualizaron reportes de benchmark:
- `docs/performance/generator-benchmark.json`
- `docs/performance/generator-benchmark.md`
- `docs/performance/generator-benchmark-check.md`

## Cambios realizados (P14 - slice 2: benchmark del generador integrado en CI no bloqueante)
- Se integro ejecucion del benchmark del generador en workflow CI:
- `/.github/workflows/ci.yml` ahora corre:
- `npm run perf:generator`
- `npm run perf:generator:check` (modo no bloqueante)
- Se publican artefactos de benchmark para inspeccion post-run:
- `docs/performance/generator-benchmark.json`
- `docs/performance/generator-benchmark.md`
- `docs/performance/generator-benchmark-check.md`

## Cambios realizados (P14 - slice 3: umbrales y politica documentados para endurecer gate)
- Se actualizaron criterios y protocolo en:
- `docs/performance-sla.md`
- Politica actual definida en baseline (`generator-baseline.json`):
- `maxP95RegressionPct: 25`
- `maxP50RegressionPct: 25`
- Se documento modo estricto opcional (`PERF_GENERATOR_STRICT=1`) para transicionar a gate bloqueante cuando se estabilice variabilidad.

## Estado P14
- P14 finalizado (slices 1 a 3 completos y documentados).

## Cambios realizados (P15 - slice 1: inventario de fuentes de irregularidades y duplicados)
- Se implemento auditoria automatica para mapear fuentes activas de reglas/irregularidades:
- `scripts/auditIrregularSources.mjs`
- Nuevo comando:
- `npm run audit:irregular-sources`
- La auditoria genera inventario versionable en:
- `docs/linguistic/irregular-sources-inventory.json`
- `docs/linguistic/irregular-sources-inventory.md`
- Hallazgos concretos registrados:
- solapamiento alto entre fuentes tecnicas y learning (`94` lemmas en comun por ejemplos).
- divergencias en gerundios entre `irregularPatterns` y `nonfiniteBuilder` (muchos lemmas solo en builder).
- divergencias en participios entre `irregularPatterns`, `nonfiniteBuilder` y `conjugationRules`.
- Se deja base objetiva para avanzar la consolidacion canonical de Fase 4.1.

## Cambios realizados (P15 - slice 2: definicion de estructura canonical unica)
- Se definio el dataset canónico v1 para irregularidades en:
- `src/lib/data/irregularCanonical.js`
- Estructura canonical incorporada:
- `nonfinite.gerund[lemma]`
- `nonfinite.participle[lemma] -> { primary, alternates }`
- `finite.futureConditionalRoots[lemma] -> { root, group }`
- API utilitaria expuesta:
- `getCanonicalGerund`
- `getCanonicalParticiple`
- `getCanonicalFutureRoot`
- `validateIrregularCanonical`
- Se documento el contrato canonical y criterio de migracion en:
- `docs/linguistic/irregular-canonical-schema.md`
- Tests agregados:
- `src/lib/data/irregularCanonical.test.js`

## Cambios realizados (P15 - slice 3: migracion inicial a fuente canonical con no-regresion)
- Se migro un subconjunto inicial (verbos/familias no finitas de alta frecuencia) para consumir la fuente canónica:
- `src/lib/core/nonfiniteBuilder.js` ahora prioriza:
- `getCanonicalGerund`
- `getCanonicalParticiple`
- Se mantiene mapa local extendido como fallback para cubrir casos no migrados aun.
- `src/lib/core/conjugationRules.js` (`hasIrregularParticiple`) ahora consulta primero el canonical dataset y luego fallback legacy extendido.
- Se agregaron pruebas de no regresion para la migracion inicial:
- `src/lib/core/nonfiniteBuilder.test.js`
- `src/lib/core/conjugationRules.nonfinite.test.js`

## Estado P15
- P15 finalizado (slices 1 a 3 completos y documentados).

## Cambios realizados (P16 - slice 1: fixtures estables para irregularidades top y edge cases)
- Se incorporaron fixtures estables para validacion linguistica de irregularidades en:
- `src/lib/data/fixtures/irregularGoldenFixtures.js`
- Cobertura inicial incluida:
- no finitos irregulares (gerundio + participio) para verbos top.
- preteritos fuertes/supletivos en `la_general` para verbos criticos.
- Se agrego test de estructura/consistencia de fixtures:
- `src/lib/data/fixtures/irregularGoldenFixtures.test.js`

## Cambios realizados (P16 - slice 2: golden tests de no-finitos y preteritos fuertes)
- Se agregaron golden tests de regresion basados en fixtures:
- `src/lib/core/irregularGolden.nonfinite.test.js`
- `src/lib/core/irregularGolden.preterite.test.js`
- Los tests verifican:
- match exacto de gerundio/participio esperado.
- match de familias esperadas para preteritos fuertes.
- match de formas `pretIndef` (`1s`, `3s`, `3p`) contra dataset real de `verbDataService`.
- Tambien se reforzaron tests de migracion inicial canonical:
- `src/lib/core/nonfiniteBuilder.test.js`
- `src/lib/core/conjugationRules.nonfinite.test.js`

## Cambios realizados (P16 - slice 3: comparador before/after para regresion linguistica)
- Se agrego comparador automatizado de fixtures vs salida actual:
- `scripts/compareIrregularGolden.mjs`
- Nuevo comando:
- `npm run audit:irregular-golden`
- El comparador genera reporte versionable en:
- `docs/linguistic/irregular-golden-report.json`
- `docs/linguistic/irregular-golden-report.md`
- Baseline actual: `Checks 44`, `Mismatches 0`.

## Estado P16
- P16 finalizado (slices 1 a 3 completos y documentados).

## Cambios realizados (P17 - slice 1: reporte linguistico publicado en CI)
- Se integro publicacion del reporte de regresion linguistica en CI:
- `/.github/workflows/ci.yml` ejecuta `npm run audit:irregular-golden`.
- Se suben artefactos del reporte:
- `docs/linguistic/irregular-golden-report.json`
- `docs/linguistic/irregular-golden-report.md`

## Cambios realizados (P17 - slice 2: criterio de severidad para mismatches linguísticos)
- Se extendio el comparador `scripts/compareIrregularGolden.mjs` con severidad por mismatch:
- `critical`: faltante de forma o valor incorrecto en golden preterite.
- `high`: mismatch en gerundio/participio de fixtures.
- `medium`: desvio de familia esperada.
- El reporte ahora incluye `severitySummary` (`critical/high/medium`).

## Cambios realizados (P17 - slice 3: modo estricto opcional para gate gradual)
- Se habilito modo estricto por entorno en el comparador:
- `IRREGULAR_GOLDEN_STRICT=1`
- CI ahora lo consume via variable de repositorio:
- `vars.IRREGULAR_GOLDEN_STRICT` (default `0`).
- En `status-check`, si `IRREGULAR_GOLDEN_STRICT=1` y falla el job de performance/linguistic checks, el pipeline pasa a fallar.

## Estado P17
- P17 finalizado (slices 1 a 3 completos y documentados).

## Cambios realizados (P18 - slice 1: dashboard con primera vista de alto impacto)
- Se redujo la primera vista del dashboard para priorizar bloques accionables:
- secciones avanzadas pasan a carga bajo demanda detrás de toggle.
- `ProgressOverview`, plan diario, modos de práctica, mapa de calor y smart practice quedan en primera vista.
- Secciones de menor impacto inmediato (`LearningJourney`, `ProgressUnlocks`, `AccuracyTrend`, `PronunciationStats`, `ErrorIntelligence`, `StudyInsights`) quedan agrupadas en modo avanzado.
- Archivo principal:
- `src/features/progress/ProgressDashboard.jsx`

## Cambios realizados (P18 - slice 2: CTA accionable persistente en vista principal)
- Se agrego barra de acciones rápidas en primera vista:
- `Practicar ahora`
- `Repasar SRS`
- `Actualizar progreso`
- Esto asegura salida accionable inmediata sin depender de secciones secundarias.
- Estilos:
- `src/features/progress/progress-streamlined.css`

## Cambios realizados (P18 - slice 3: medicion e instrumentacion de primera vista)
- Se agrego instrumentacion runtime en `ProgressDashboard`:
- log `Progress first view ready` con `elapsedMs` cuando la primera vista queda lista.
- Se documento protocolo de validacion e impacto esperado en:
- `docs/performance/progress-dashboard-first-view.md`
- Tests actualizados:
- `src/features/progress/ProgressDashboard.smoke.test.jsx`
- `src/features/progress/ProgressDashboard.navigation.test.jsx`

## Estado P18
- P18 finalizado (slices 1 a 3 completos y documentados).

## Cambios realizados (P19 - slice 1: contrato de dependencias cache/invalidez documentado)
- Se documento contrato operativo de cache del dashboard en:
- `docs/performance/progress-cache-contract.md`
- Incluye:
- tipos de dato y TTL por bloque,
- mapeo de eventos `progress:*` a invalidacion parcial,
- dependencias por seccion del dashboard.

## Cambios realizados (P19 - slice 2: TTL explicito para bloques del dashboard)
- Se formalizo politica TTL unificada en `ProgressDataCache`:
- `PROGRESS_CACHE_TTL_MS` ahora cubre core + heavy analytics + bloques dinamicos.
- Se extendio `useProgressDashboardData` para cachear tambien secciones heavy con `progressDataCache.get`:
- `studyPlan`
- `advancedAnalytics`
- `community`
- `offlineStatus`
...
## Cambios realizados (P19 - slice 3: tests de invalidez por evento y contrato de cache)
- Se amplio cobertura en:
- `src/lib/cache/ProgressDataCache.test.js`
- Validaciones agregadas:
- politica TTL explicita por data type.
- mapeo de eventos a keys (`settings_change`, `challenge_completed`, etc.).
- resolucion de invalidacion parcial vs full refresh.
- Se mantuvo cobertura de eventos `progress:*`/sync en:
- `src/features/progress/useProgressDashboardData.test.js`

## Estado P19
- P19 finalizado (slices 1 a 3 completos y documentados).

## Cambios realizados (P20 - slice 1: diferir recomputos secundarios del dashboard)
- Se habilito carga diferida de analíticas secundarias en el hook:
- `useProgressDashboardData({ enableSecondaryData })`
- Con `enableSecondaryData=false`, se cargan solo claves primarias de primera vista.
- Al habilitar modo avanzado, se cargan secundarias pendientes sin refresh completo.
- Se integró desde `ProgressDashboard` con:
- `useProgressDashboardData({ enableSecondaryData: showAdvancedSections })`

## Cambios realizados (P20 - slice 2: refresh parcial sin reprocesar bloques no impactados)
- Se reforzo `refreshFromEvent` para filtrar keys secundarias cuando el modo avanzado no está activo.
- Se mantiene invalidacion parcial por evento y se evita recomputo de bloques secundarios no afectados.
- Se ampliaron tests de hook para cubrir este contrato incremental.

## Cambios realizados (P20 - slice 3: medicion incremental vs full refresh)
- Se documentó comparación operativa en:
- `docs/performance/progress-refresh-measurement.md`
- Cobertura de medición en tests:
- `src/features/progress/useProgressDashboardData.test.js`
- casos cubiertos:
- refresh parcial por `attemptId` (sin analíticas pesadas),
- refresh completo por `sync`,
- diferido de secundarias hasta activar modo avanzado.

## Estado P20
- P20 finalizado (slices 1 a 3 completos y documentados).

## Cambios realizados (P21 - slice 1: suite mínima de chequeos a11y para dashboard y drill)
- Se incorporó suite automática mínima de accesibilidad con `axe-core`:
- `src/features/progress/ProgressDashboard.a11y.test.jsx`
- `src/components/drill/DrillMode.a11y.test.jsx`
- Nuevo comando:
- `npm run test:a11y`

## Cambios realizados (P21 - slice 2: ejecución de axe en componentes críticos)
- Se integró ejecución de `axe.run` sobre render real de componentes críticos.
- Regla `color-contrast` queda desactivada en jsdom (limitación técnica conocida), manteniendo el resto de chequeos semánticos y de landmarks.
- Dependencia agregada:
- `axe-core` (devDependency).

## Cambios realizados (P21 - slice 3: backlog a11y documentado con severidad)
- Se documentó estado de hallazgos y riesgos residuales en:
- `docs/accessibility/a11y-backlog.md`
- Estado actual registrado:
- sin hallazgos `critical/high/medium` en primera pasada automática.
- riesgos residuales explicitados para contraste real y navegación de teclado end-to-end.

## Estado P21
- P21 finalizado (slices 1 a 3 completos y documentados).

## Cambios realizados (P22 - slice 1: auditoria de foco en overlays de Drill)
- Se auditó y ajustó manejo de foco en `DrillMode` para overlays:
- captura de foco al abrir paneles,
- restauración de foco al trigger al cerrar paneles.
- Archivo:
- `src/components/drill/DrillMode.jsx`

## Cambios realizados (P22 - slice 2: comportamiento Escape/tab en paneles críticos)
- Se agregó cierre por `Escape` para overlays activos de Drill (`quick switch`, `games`, `pronunciation`).
- Se mejoró semántica/teclado en `DrillHeader`:
- `type=\"button\"` explícito,
- `aria-label` consistente en icon buttons.
- Archivos:
- `src/components/drill/DrillMode.jsx`
- `src/components/drill/DrillHeader.jsx`

## Cambios realizados (P22 - slice 3: tests de teclado en flujo básico Drill)
- Se agregaron tests de teclado:
- `src/components/drill/DrillMode.keyboard.test.jsx`
- Cobertura:
- cerrar quick switch con `Escape`,
- restaurar foco al botón disparador,
- orden base de tab en header de Drill.
- Se actualizó backlog a11y:
- `docs/accessibility/a11y-backlog.md`

## Estado P22
- P22 finalizado (slices 1 a 3 completos y documentados).

## Cambios realizados (P23 - slice 1: barrido a11y en ProgressDashboard con modo avanzado)
- Se amplió suite a11y de dashboard con escenario de secciones avanzadas activadas:
- `src/features/progress/ProgressDashboard.a11y.test.jsx`
- Cobertura:
- primera vista,
- modo avanzado tras toggle (`Ver análisis avanzados`).

## Cambios realizados (P23 - slice 2: plan de contraste en browser real)
- Se definió plan operativo para cubrir contraste y navegación real fuera de jsdom:
- `docs/accessibility/a11y-browser-plan.md`
- Incluye estrategia Playwright + axe y gate gradual (`critical` -> `critical+high`).

## Cambios realizados (P23 - slice 3: reporte a11y como artefacto en CI)
- Se agregó generación de reporte a11y:
- `scripts/generateA11yReport.mjs`
- nuevo comando: `npm run report:a11y`
- CI actualizado para ejecutar y publicar artefactos a11y:
...
## Estado P23
- P23 finalizado (slices 1 a 3 completos y documentados).
...
- `npx vitest run src/hooks/modules/formsPoolService.test.js src/hooks/modules/DrillFormFilters.test.js src/hooks/modules/useDrillGenerator.generateNextItem.test.js`
- `npx eslint src/hooks/modules/formsPoolService.js src/hooks/modules/formsPoolService.test.js src/hooks/modules/DrillFormFilters.js src/hooks/modules/DrillFormFilters.test.js src/hooks/modules/useDrillGenerator.js`
- `npm run perf:generator`
...
## Hotfix crítico - Subjuntivo imperfecto (2026-02-12)

### Problema reportado
- En `subjImpf` para ciertos verbos `-ar` con diptongación (`recordar`, `costar`, `sonar`, `volar`, `despertar`) se estaban sirviendo formas truncadas (`costra`, `volras`, `despertra`, etc.).
- Impacto: corrección de respuestas rota en ejercicios de subjuntivo imperfecto.

### Cambios hechos
- Corregidas las 35 formas afectadas en `src/data/verbs.js` (7 personas x 5 verbos).
- Agregado test de regresión específico en `src/data/verbs.subjImpf-regression.test.js` para bloquear reintroducción del bug.

### Verificación
- `npx vitest run src/data/verbs.subjImpf-regression.test.js` ✅

## Hotfix crítico - Auditoría DB + reglas (2026-02-12)

### Auditoría ejecutada
- Se corrieron auditorías existentes (`validate-integrity`, `audit:all`, `audit:irregular-sources`, `audit:irregular-golden`) y tests core del generador/reglas.
- Se agregó chequeo derivacional específico de `subjImpf` (desde `pretIndef` 3p) para detectar corrupción de formas.
- Reporte: `docs/linguistic/db-rules-audit-2026-02-12.md`.

### Problemas críticos encontrados y corregidos
- `src/data/verbs.js` tenía corrupción en `subjImpf` (28 formas) para:
- `preferir`, `resolver`, `competir`, `vestir`.
- Además se corrigieron typos de base en pretérito de `preferir` (`prefirió/prefirieron`) y en 3ª persona de `competir` (`compitió/compitieron`) y `vestir` (`vistió/vistieron`).

### Guardrails nuevos
- Nuevo test global de integridad: `src/data/verbs.subjImpf-integrity.test.js`.
- Este test valida para todo el dataset que `subjImpf` sea coherente con la raíz de `pretIndef` 3p (aceptando variantes `-ra`/`-se`, acento-insensitive).

### Verificación
- `npx vitest run src/data/verbs.subjImpf-regression.test.js src/data/verbs.subjImpf-integrity.test.js` ✅

## Cambios realizados (Fase 7.1 - slice 1: estabilidad de refresh en SmartPractice)
- `SmartPractice` ahora debouncea cambios de `recommendationKey` (200ms) antes de disparar recomputación ML.
- Objetivo: evitar thrash y llamadas redundantes cuando `userStats` cambia en ráfaga por filtros rápidos.
- Se mantuvo el refresh por `progress:dataUpdated` con debouncer existente (250ms), combinando ambas protecciones.
- Nuevo test: `debounces ML recomputation when user stats change rapidly` en `src/features/progress/SmartPractice.test.jsx`.
- Verificación: `npx vitest run src/features/progress/SmartPractice.test.jsx` ✅.

## Cambios realizados (Fase 7.1 - slice 2: keys estables en listas dinámicas)
- Se eliminó uso de `key={index}` en listas dinámicas de componentes de progreso con interacción frecuente:
- `src/features/progress/ErrorPatternRecognition.jsx`
- `src/features/progress/PracticeRecommendations.jsx`
- `src/features/progress/GeneralRecommendations.jsx`
- Se incorporó helper reutilizable `buildStableListKey` en:
- `src/features/progress/progressListKeys.js`
- Se agregaron tests unitarios del helper:
- `src/features/progress/progressListKeys.test.js`
- Impacto esperado: menor riesgo de remount innecesario y mejor preservación de estado/render al reordenar o refrescar recomendaciones.
- Verificación: `npx vitest run src/features/progress/progressListKeys.test.js src/features/progress/GeneralRecommendations.test.jsx src/features/progress/SmartPractice.test.jsx` ✅.

## Estado consolidado (para retomar rápido)

### Ya resuelto en esta tanda
- Integridad crítica de conjugaciones `subjImpf` restaurada para casos reportados y casos adicionales detectados en auditoría.
- Guardrails de datos añadidos:
- `src/data/verbs.subjImpf-regression.test.js`
- `src/data/verbs.subjImpf-integrity.test.js`
- Auditoría técnica documentada en:
- `docs/linguistic/db-rules-audit-2026-02-12.md`
- SmartPractice endurecido contra inestabilidad de refresco:
- debounce de recomputación ML por cambios rápidos de stats.
- reducción de remounts por keys inestables en listas dinámicas de progreso.

### Pendiente inmediato (siguiente slice activo)
- Fase 7.1 - slice 3: medir impacto real en re-render del dashboard ante cambios rápidos.
- Objetivo de salida:
- baseline reproducible antes/después,
- métrica de renders/recomputes y tiempo de refresco,
- criterio explícito de aceptación para no degradar UX.
- Entregables recomendados:
- test/medición en `src/features/progress/useProgressDashboardData.test.js` o script dedicado en `scripts/`,
- reporte en `docs/performance/progress-refresh-measurement.md` con números before/after,
- si corresponde, ajuste fino de dependencias/memoización en `SmartPractice` y consumidores del dashboard.

### Pendientes próximos después de 7.1
- Fase 7.2 (seguridad/privacidad):
- eliminar dependencia de secretos de sync en frontend cuando aplique,
- definir y documentar token exchange seguro vía backend.
- Fase 8 (operación/CI):
- endurecer presupuestos de performance en CI (de no bloqueante a bloqueante donde aplique),
- hygiene final de artefactos legacy/backups,
- formato fijo de changelog técnico por release.
