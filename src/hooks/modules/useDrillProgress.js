...
export const useDrillProgress = () => {
  ...
  const handleResponse = useCallback(async (item, response, onResult) => {
    ...
    try {
      ...
      // Check if progress system is ready (simplified check)
  ...
  const getProgressInsights = useCallback(() => {
    return {
      ...
      momentum,
      confidence,
      isSystemReady: !!processUserResponse,
  ...
  }, [progressStats, flowState, momentum, confidence])
  ...
  // Initialize progress system readiness check
  useEffect(() => {
